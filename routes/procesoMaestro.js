...
router.patch('/:ProcesoMaestroId', async (req,res) => {
    try {        
        const updatedProcesoMaestro = await ProcesoMaestro.updateOne(
            ...
            { $set: { codigoProcesoMaestro: req.body.codigoProcesoMaestro} },
            { $set: { ordenPedido: req.body.ordenPedido} },
            { $set: { fechaOrden: req.body.fechaOrden} },
            { $set: { proovedor: req.body.proovedor} },
            { $set: { comprobante: req.body.comprobante} }
        );
